...
Usage:
  eam -h | --help
  eam (-n | -f | -c | -e <experiment> | -r | -d) [--domain=DOMAIN] [--runpath=PATH ] [ -l (en | es) ]

Options:
  -h    Show this screen.
  -n    Trains the neural network (autoencoder).
  -f    Generates Features for all data using the encoder.
  -c    Trains the neural network (classifier)
...
def maximum(arrays):
    ...
    return np.max(global_max, axis=0)
    # max = float('-inf')
    # for a in arrays:
    #     local_max = np.max(a)
    #     if local_max > max:
...
def minimum(arrays):
    ...
    return np.min(global_min, axis=0)
    # min = float('inf')
    # for a in arrays:
    #     local_min = np.min(a)
    #     if local_min < min:
...
def get_ams_results(
    ...
    # Round the values
    max_value = maximum((trf)) #maximum((trf, tef))
    min_value = minimum((trf)) #minimum((trf, tef))
    ...
    precision = behaviour[constants.correct_response_idx]/float(responses) if responses > 0 else 0.0
...
def test_filling_per_fold(mem_size, domain, es, fold):
    ...
    max_value = maximum((filling_features)) #maximum((filling_features, testing_features))
    min_value = minimum((filling_features)) #minimum((filling_features, testing_features))
...
def remember(msize, mfill, es):
    ...
    for sigma in constants.sigma_values:
        print(f'Running remembering for sigma = {sigma:.3f}')
        ...
        #
        partial_memories_prefix = constants.partial_memories_name(es) + suffix
        partial_recog_prefix = constants.partial_recog_name(es) + suffix
        partial_weights_prefix = constants.partial_weights_name(es) + suffix
        partial_classif_prefix = constants.partial_classification_name(es) + suffix
        #
        prefixes_list = [
            ...
            [partial_memories_prefix, partial_recog_prefix,
                partial_weights_prefix, partial_classif_prefix]
        ]

        for fold in range(constants.n_folds):
            print(f'Running remembering for sigma = {sigma:.3f} and fold: {fold} with memory filled at {mfill}%\n')
            #
            suffix = constants.filling_suffix
            filling_features_filename = constants.features_name(es) + suffix
            filling_features_filename = constants.data_filename(
            ...
            partial_features_filename = constants.features_name(es) + suffix
            partial_features_filename = constants.data_filename(
                partial_features_filename, es, fold)
            #
            filling_features = np.load(filling_features_filename)
            testing_features = np.load(testing_features_filename)
            ...
            partial_rounded = msize_features(
                partial_features, msize, min_value, max_value)
            ...
            for features, prefixes in zip(
                    [testing_rounded, noised_rounded, partial_rounded], prefixes_list):
                remember_with_sigma(eam, features, prefixes,
                                    msize, min_value, max_value, es, fold)
    print('Remembering done!')


# Obtain reconstruction, recognition value and weights from memory.
...
def decode_test_features(es):
    ...
    # Features prefix of original testing images
    suffix = constants.testing_suffix
    testing_features_prefix = constants.features_prefix + suffix

    # Features prefix of noised testing images
    suffix = constants.noised_suffix
    noised_features_prefix = constants.features_prefix + suffix

    # Features prefix of partial testing images
    suffix = constants.partial_suffix
    partial_features_prefix = constants.features_prefix + suffix
    ...
    for fold in range(constants.n_folds):
        ...
        testing_features = np.load(testing_features_filename)
        # Load noised test features
        noised_features_filename = constants.data_filename(
            noised_features_prefix, es, fold)
        noised_features = np.load(noised_features_filename)
        # Load partial test features
        partial_features_filename = constants.data_filename(
            partial_features_prefix, es, fold)
        partial_features = np.load(partial_features_filename)

        # Get raw data
        testing_data, testing_labels = dataset.get_testing(fold) # <-- Original raw data
        noised_data, _ = dataset.get_testing(fold, noised=True) # <-- Noised raw data
        partial_data, _ = dataset.get_testing(fold, partial=True) # <-- Partial raw data
        ...
        prod_part_images = model.predict(partial_features)
        n = len(testing_labels)

        for (i, testing, prod_test, noised, prod_noise, partial, prod_partial, label) in \
                zip(range(n), testing_data, prod_test_images, noised_data,
                    prod_nsed_images, partial_data, prod_part_images, testing_labels):
            store_original_and_test(
                testing, prod_test,
                noised, prod_noise,
                partial, prod_partial,
...
def decode_memories(msize, es):
    ...
    for sigma in constants.sigma_values:
        ...
        partial_prefix = constants.partial_memories_name(es) + suffix
        for fold in range(constants.n_folds):
            ...
            partial_images = model.predict(partial_features)
            n = len(testing_labels)
            memories_path = constants.memories_path + suffix
            for (i, memory, noised, partial, label) in \
                    zip(range(n), memories_images, noised_images, partial_images,
                        testing_labels):
                store_memory(memory, memories_path, i, label, es, fold)
                store_noised_memory(noised, memories_path, i, label, es, fold)
                store_partial_memory(partial, memories_path, i, label, es, fold)


def store_original_and_test(testing, prod_test,
                            noised, prod_noise,
                            partial, prod_partial,
    ...
    prod_noise_filename = constants.prod_noised_image_filename(
        directory, idx, label, es, fold)
    partial_filename = constants.partial_image_filename(
        directory, idx, label, es, fold)
    prod_part_filename = constants.prod_partial_image_filename(
        directory, idx, label, es, fold)
    ...
    store_image(partial_filename, partial)
    store_image(prod_part_filename, prod_partial)
...
def store_partial_memory(memory, directory, idx, label, es, fold):
    memory_filename = constants.partial_image_filename(
        directory, idx, label, es, fold)
    store_image(memory_filename, memory)
...
def store_image(filename, array):
    pixels = array.reshape(dataset.columns, dataset.rows*3) * 255
...
def dreaming_per_fold(features, chosen, eam, min_value, max_value,
    ...
    unknown = np.zeros((dataset.rows, dataset.columns, 3), dtype=int)
    suffix = constants.noised_suffix if noised else ''
    suffix += constants.msize_suffix(msize)
    classification = []
    for sigma in [0.035,0.04,0.045,0.055]:# Changed 2023.10.23 constants.sigma_values:
        ...
        for i in range(cycles):
            ...
            if not recognized:
                store_dream(unknown, *chosen[fold], full_suffix, es, fold)
                classification.append(10)
                continue
            image = decoder(np.array([dream,]), training=False)[0].numpy() 
            classif = np.argmax(classifier(np.array([dream,]), training=False), axis=1)[0] 
            classification.append(classif)
            store_dream(image, *chosen[fold], full_suffix, es, fold)
            ft_cycle = encoder(np.array([image,]), training=False)[0].numpy() 
...
def create_and_train_network_ae(es):
    model_prefix = constants.model_name(es)
    stats_prefix = model_prefix + constants.autoencoder_suffix
    history = neural_net.train_network_ae(model_prefix, es)
    save_history(history, stats_prefix, es)

def create_and_train_network_cla(es):
    model_prefix = constants.model_name(es)
    stats_prefix = model_prefix + constants.classifier_suffix
    history, conf_matrix = neural_net.train_network_cla(model_prefix, es)
    save_history(history, stats_prefix, es)
...
if __name__ == "__main__":
    ...
    if args['-n']:
        create_and_train_network_ae(exp_settings)
    elif args['-c']:
        create_and_train_network_cla(exp_settings)
    elif args['-f']:
