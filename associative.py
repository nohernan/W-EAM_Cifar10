...
class AssociativeMemory(object):
    ...
    def containment(self, r_io):
        ...
        imp = ~r_io[:self.m, :] | self.iota_relation
        # The zeros in sum mean undefined cells in r_io. i.e., values
        # less than 0 or greater than m-1
        sum = np.sum(r_io[:self.m, :], axis=0)
        # Since the zeros in imp and sum cannot occur at the same time,
