...
prod_partial_suffix = '-prod_partial'
...
sigma_values = [0.01, 0.03, 0.05, 0.07, 0.09, 0.11]
...
def partial_recog_name(es):
    return recog_partial_prefix
...
def partial_weights_name(es):
    return weights_partial_prefix
...
def partial_classification_name(es):
    return classification_partial_prefix
...
def create_directory(path):
    try:
        ...
        pass # print(f'Directory {path} already exists.')
...
def partial_image_filename(dir, idx, label, es, fold):
    return image_filename(dir, idx, label, partial_suffix, es, fold)

def prod_partial_image_filename(dir, idx, label, es, fold):
    return image_filename(dir, idx, label, prod_partial_suffix, es, fold)
